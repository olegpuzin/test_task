...
function Pagination({ page, setPage, countPage }) {
    ...
    return (
        <div className='pagination'>
            {page > 1 && <button onClick={() => dispatch(setPage(1))} className='pageNumber start'>1</button>}
            ...
            {page < countPage - 1 && <button onClick={() => dispatch(setPage(countPage))} className='pageNumber end'>{countPage}</button>}
        </div>
    )
}
